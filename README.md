...
| [live-view-vs-cdp-screencast](live-view-vs-cdp-screencast) | Watch live view and a CDP `Page.startScreencast` side by side in one page. The screencast streams straight from the browser tab with no relay server, and forwards mouse and keyboard back over CDP `Input.*`. |
